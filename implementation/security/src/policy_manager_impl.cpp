...
policy_manager_impl::policy_manager_impl() :
    ...
    policy_enabled_(false), check_credentials_(false), allow_remote_clients_(true), check_whitelist_(false), policy_base_path_(""),
    check_routing_credentials_(false),
#endif // !VSOMEIP_DISABLE_SECURITY
    is_configured_(false) {
}

bool policy_manager_impl::is_enabled() const {
...
bool policy_manager_impl::is_audit() const {
#ifdef VSOMEIP_DISABLE_SECURITY
    return false;
#else
...
bool policy_manager_impl::check_credentials(client_t _client, const vsomeip_sec_client_t* _sec_client) {
    ...
    std::shared_lock<shared_mutex> its_lock(any_client_policies_mutex_);
    for (const auto& p : any_client_policies_) {
        ...
        if ((has_id && p->allow_who_) || (!has_id && !p->allow_who_)) {
            ...
            if (!store_client_to_sec_client_mapping(_client, _sec_client)) {
                ...
                VSOMEIP_INFO << "vSomeIP Security: Client 0x" << std::hex << _client << " with UID/GID=" << std::dec << its_uid << "/"
                             << its_gid
                             << " : Check credentials failed as existing credentials would be "
                                "overwritten"
                             << security_mode_text;
                return !check_credentials_;
            }
    ...
    VSOMEIP_INFO << "vSomeIP Security: Client 0x" << std::hex << _client << " with UID/GID=" << std::dec << its_uid << "/" << its_gid
...
bool policy_manager_impl::check_routing_credentials(const vsomeip_sec_client_t* _sec_client) const {
    ...
    if (_sec_client && _sec_client->port == VSOMEIP_SEC_PORT_UNUSED) {
        ...
        if (routing_credentials_.first == its_uid && routing_credentials_.second == its_gid) {
    ...
    VSOMEIP_INFO << "vSomeIP Security: UID/GID=" << (is_known_uid_gid ? std::to_string(its_uid) : "n/a") << "."
                 << (is_known_uid_gid ? std::to_string(its_gid) : "n/a") << " : Check routing credentials failed as "
                 << "configured routing manager credentials "
                 << "do not match with routing manager credentials" << security_mode_text;
...
bool policy_manager_impl::is_client_allowed(const vsomeip_sec_client_t* _sec_client, service_t _service, instance_t _instance,
    ...
    {
        std::shared_lock<shared_mutex> its_cache_lock(is_client_allowed_cache_mutex_);
    ...
    for (const auto& p : any_client_policies_) {
        ...
        if ((has_uid && has_gid && p->allow_who_) || ((!has_uid || !has_gid) && !p->allow_who_)) {
            if (p->allow_what_) {
                // allow policy
                if (is_matching) {
                    std::unique_lock<shared_mutex> its_cache_lock(is_client_allowed_cache_mutex_);
    ...
    VSOMEIP_INFO << "vSomeIP Security: UID/GID=" << std::dec << its_uid << "/" << its_gid
                 << " : Isn't allowed to communicate with service/instance/(method / event) " << std::hex << _service << "/" << _instance
...
bool policy_manager_impl::is_offer_allowed(const vsomeip_sec_client_t* _sec_client, service_t _service, instance_t _instance) const {
    ...
    VSOMEIP_INFO << "vSomeIP Security: UID/GID=" << std::dec << its_uid << "/" << its_gid << " isn't allowed to offer service/instance "
                 << std::hex << _service << "/" << _instance << security_mode_text;
...
#ifndef VSOMEIP_DISABLE_SECURITY
void policy_manager_impl::load(const configuration_element& _element, const bool _lazy_load) {
...
bool policy_manager_impl::remove_security_policy(uid_t _uid, gid_t _gid) {
    std::unique_lock<shared_mutex> its_lock(any_client_policies_mutex_);
        ...
        if (p->credentials_.size() == 1) {
            ...
            if (its_uids.first.lower() == _uid && its_uids.first.upper() == _uid) {
                if (its_uids.second.size() == 1) {
                    ...
                    if (its_gids.lower() == _gid && its_gids.upper() == _gid) {
    ...
    if (its_matching_policy) {
        ...
        for (const auto& r : _policy->requests_) {
            ...
            for (auto s = its_lower; s <= its_upper; s++) {
                boost::icl::discrete_interval<service_t> its_service(s, s, boost::icl::interval_bounds::closed());
...
void policy_manager_impl::add_security_credentials(uid_t _uid, gid_t _gid, const std::shared_ptr<policy>& _policy, client_t _client) {
    ...
    if (!was_found) {
        any_client_policies_.push_back(_policy);
        VSOMEIP_INFO << __func__ << " Added security credentials at client: 0x" << std::hex << _client << std::dec << " with UID: " << _uid
                     << " GID: " << _gid;
    }
}

bool policy_manager_impl::is_policy_update_allowed(uid_t _uid, std::shared_ptr<policy>& _policy) const {
    ...
    if (is_uid_allowed && _policy) {
        ...
        for (const auto& its_request : _policy->requests_) {
            ...
            for (its_service = its_request.first.lower(); its_service <= its_request.first.upper(); its_service++) {
            ...
            if (!has_service) {
                if (!check_whitelist_) {
                    VSOMEIP_INFO << "vSomeIP Security: Policy update requesting service ID: " << std::hex << its_service
                                 << " is not allowed, but will be allowed due to whitelist audit "
                                    "mode is active!";
                } else {
                    VSOMEIP_WARNING << "vSomeIP Security: Policy update requesting service ID: " << std::hex << its_service
        ...
        if (!check_whitelist_) {
            VSOMEIP_INFO << "vSomeIP Security: Policy update for UID: " << std::dec << _uid
                         << " is not allowed, but will be allowed due to whitelist audit mode is "
                            "active!";
        } else {
...
bool policy_manager_impl::parse_policy(const byte_t*& _buffer, uint32_t& _buffer_size, uid_t& _uid, gid_t& _gid,
...
void policy_manager_impl::load_policies(const configuration_element& _element) {
    try {
        ...
        for (auto its_security = found_policy.begin(); its_security != found_policy.end(); ++its_security) {
            if (its_security->first == "check_credentials") {
                ...
                for (auto its_policy = its_security->second.begin(); its_policy != its_security->second.end(); ++its_policy) {
...
void policy_manager_impl::load_policy(const boost::property_tree::ptree& _tree) {
                ...
                if (its_key == "uid") {
                    if (n->second.data().empty()) {
                        ...
                        if (its_value != "any") {
                            ...
                            its_uid_interval = boost::icl::construct<boost::icl::discrete_interval<uid_t>>(
                                    its_uid, its_uid, boost::icl::interval_bounds::closed());
                        } else {
                            its_uid_interval = boost::icl::construct<boost::icl::discrete_interval<uid_t>>(
                                    std::numeric_limits<uid_t>::min(), std::numeric_limits<uid_t>::max(),
                    ...
                    if (n->second.data().empty()) {
                        ...
                        if (its_value != "any") {
                            ...
                            its_gid_interval = boost::icl::construct<boost::icl::discrete_interval<gid_t>>(
                                    its_gid, its_gid, boost::icl::interval_bounds::closed());
                        } else {
                            its_gid_interval = boost::icl::construct<boost::icl::discrete_interval<gid_t>>(
                                    std::numeric_limits<gid_t>::min(), std::numeric_limits<gid_t>::max(),
                                    boost::icl::interval_bounds::closed());
                        }
                        has_gid = true;
                    }
    ...
    for (auto l = _tree->second.begin(); l != _tree->second.end(); ++l) {
        if (l->first == "requests") {
            for (auto n = l->second.begin(); n != l->second.end(); ++n) {
                ...
                boost::icl::interval_map<instance_t, boost::icl::interval_set<method_t>> its_instance_method_intervals;
                for (auto k = n->second.begin(); k != n->second.end(); ++k) {
                    if (k->first == "service") {
                        ...
                        boost::icl::discrete_interval<instance_t> all_instances(0x01, 0xFFFF, boost::icl::interval_bounds::closed());
                        boost::icl::discrete_interval<method_t> all_methods(0x01, 0xFFFF, boost::icl::interval_bounds::closed());
                ...
                if (its_service != 0x0 && !its_instance_method_intervals.empty()) {
                    _policy->requests_ += std::make_pair(
                            boost::icl::discrete_interval<service_t>(its_service, its_service, boost::icl::interval_bounds::closed()),
                ...
                for (auto k = n->second.begin(); k != n->second.end(); ++k) {
                    if (k->first == "service") {
                        ...
                        if (its_value != "any") {
                            ...
                            its_instance_interval_set.insert(boost::icl::discrete_interval<instance_t>(0x0001, 0xFFFF));
                        }
                    } else if (k->first == "instances") { // new instances definition
...
void policy_manager_impl::load_credential(const boost::property_tree::ptree& _tree,
                                          boost::icl::interval_map<uid_t, boost::icl::interval_set<gid_t>>& _credentials) {
        ...
        for (auto j = i->second.begin(); j != i->second.end(); ++j) {
            ...
            if (its_key == "uid") {
                ...
                VSOMEIP_WARNING << "vSomeIP Security: Security configuration: "
                                << "Malformed credential (contains illegal key \"" << its_key << "\")";
            }
        }

        for (const auto its_uid_interval : its_uid_interval_set) {
            _credentials += std::make_pair(its_uid_interval, its_gid_interval_set);
        }
    }
}

bool policy_manager_impl::load_routing_credentials(const configuration_element& _element) {
    try {
...
void policy_manager_impl::load_security_update_whitelist(const configuration_element& _element) {
    try {
        ...
        for (auto its_whitelist = found_whitelist.begin(); its_whitelist != found_whitelist.end(); ++its_whitelist) {
...
void policy_manager_impl::load_security_policy_extensions(const configuration_element& _element) {
    try {
        ...
        {
            std::unique_lock<shared_mutex> its_lock(policy_extension_paths_mutex_);
        ...
        for (auto i = found_policy_extensions.begin(); i != found_policy_extensions.end(); ++i) {
            ...
            std::string its_filesystem_path = str.substr(0, str.find_last_of("\\/")) + its_path.erase(0, its_path.find_first_of("\\/"));
#endif
            if (!utility::is_folder(its_filesystem_path)) {
                VSOMEIP_DEBUG << __func__ << ": The path " << its_filesystem_path << " is not valid";
            }
            std::map<std::string, bool> empty_map;
            policy_extension_paths_[its_client_host] = std::make_pair(its_filesystem_path, empty_map);

            VSOMEIP_INFO << __func__ << ": Insert policy extension path: [" << its_filesystem_path << "] for hostname: [" << its_client_host
                         << "]";
        }
    } catch (...) { }
}
...
template<typename T_>
void policy_manager_impl::load_interval_set(const boost::property_tree::ptree& _tree, boost::icl::interval_set<T_>& _intervals,
                                            bool _exclude_margins) {
    ...
    if (its_key == "any") {
        its_intervals.insert(boost::icl::discrete_interval<T_>::closed(its_min, its_max));
    } else {
        for (auto i = _tree.begin(); i != _tree.end(); ++i) {
            auto its_data = i->second;
            if (!its_data.data().empty()) {
                ...
                for (auto j = its_data.begin(); j != its_data.end(); ++j) {
                    ...
                    if (its_key == "first") {
                        ...
                        VSOMEIP_WARNING << "vSomeIP Security: Security configuration: "
                                        << " Malformed range. Contains illegal key (" << its_key << ")";
                    }
                }
                if (has_first && has_last && its_first <= its_last) {
                    its_intervals.insert(boost::icl::discrete_interval<T_>::closed(its_first, its_last));
                }
            }
        }
    }
                        ...
                        for (const auto& j : r.second) {
                            for (const auto& k : j.second) {
                                ...
                                if (its_i_lower <= its_k_upper && its_k_lower <= its_i_upper) {
                                    ...
                                    boost::icl::interval_map<instance_t, boost::icl::interval_set<method_t>> its_instances_methods;
                                    its_instances_methods += std::make_pair(
                                            boost::icl::interval<instance_t>::closed(its_instance_min, its_instance_max), j.second);

                                    its_policy->requests_ +=
                                            std::make_pair(boost::icl::interval<instance_t>::closed(its_service_min, its_service_max),
...
bool policy_manager_impl::is_policy_extension(const std::string& _path) const {
...
std::string policy_manager_impl::get_policy_extension_path(const std::string& _client_host) const {
    std::shared_lock<shared_mutex> lock(policy_extension_paths_mutex_);
    return get_policy_extension_path_unlocked(_client_host);
}
// only be called after loading of the mutex
std::string policy_manager_impl::get_policy_extension_path_unlocked(const std::string& _client_host) const {
    std::string its_path("");
...
policy_manager_impl::policy_loaded_e policy_manager_impl::is_policy_extension_loaded(const std::string& _client_host) const {
    ...
    if (found_host != policy_extension_paths_.end()) {
        ...
        auto found_complete_path = found_host->second.second.find(get_security_config_folder(found_host->second.first));
...
void policy_manager_impl::set_is_policy_extension_loaded(const std::string& _client_host, const bool _loaded) {
    std::unique_lock<shared_mutex> lock(policy_extension_paths_mutex_);
    auto found_host = policy_extension_paths_.find(_client_host);
...
std::string policy_manager_impl::get_security_config_folder(const std::string& its_folder) const {
...
std::shared_ptr<policy> policy_manager_impl::create_policy() const {
...
bool policy_manager_impl::parse_uid_gid(const byte_t*& _buffer, uint32_t& _buffer_size, uid_t& _uid, gid_t& _gid) const {
    ...
    return (its_policy && its_policy->deserialize_uid_gid(_buffer, _buffer_size, _uid, _gid));
}

#endif // !VSOMEIP_DISABLE_SECURITY

bool policy_manager_impl::store_client_to_sec_client_mapping(client_t _client, const vsomeip_sec_client_t* _sec_client) {
    ...
    if (_sec_client != nullptr && _sec_client->port == VSOMEIP_SEC_PORT_UNUSED) {
        ...
        if (found_client != ids_.end()) {
            if (!utility::compare(found_client->second, *_sec_client)) {
                ...
                VSOMEIP_WARNING << "vSomeIP Security: Client 0x" << std::hex << _client << " with UID/GID=" << std::dec << its_new_uid
...
bool policy_manager_impl::remove_client_to_sec_client_mapping(client_t _client) {
    ...
    }
    {
        ...
        if (is_client_removed) {
            ...
            for (auto it = sec_client_to_clients_.begin(); it != sec_client_to_clients_.end(); ++it) {
...
bool policy_manager_impl::get_sec_client_to_clients_mapping(const vsomeip_sec_client_t* _sec_client, std::set<client_t>& _clients) {
