...
public:
    ...
    bool parse_uid_gid(const byte_t*& _buffer, uint32_t& _buffer_size, uid_t& _uid, gid_t& _gid) const;
    bool parse_policy(const byte_t*& _buffer, uint32_t& _buffer_size, uid_t& _uid, gid_t& _gid,
                      const std::shared_ptr<policy>& _policy) const;

    bool is_policy_update_allowed(uid_t _uid, std::shared_ptr<policy>& _policy) const;
    bool is_policy_removal_allowed(uid_t _uid) const;

    // extension
    void load(const configuration_element& _element, const bool _lazy_load = false);
    ...
    void add_security_credentials(uid_t _uid, uid_t _gid, const std::shared_ptr<policy>& _credentials_policy, client_t _client);

    void get_requester_policies(const std::shared_ptr<policy> _policy, std::set<std::shared_ptr<policy>>& _requesters) const;
...
private:
    ...
    void load_policy_body(std::shared_ptr<policy>& _policy, const boost::property_tree::ptree::const_iterator& _tree);
    void load_credential(const boost::property_tree::ptree& _tree, boost::icl::interval_map<uid_t, boost::icl::interval_set<gid_t>>& _ids);
    bool load_routing_credentials(const configuration_element& _element);
    template<typename T_>
    void load_interval_set(const boost::property_tree::ptree& _tree, boost::icl::interval_set<T_>& _range, bool _exclude_margins = false);
...
public:
    ...
    bool check_credentials(client_t _client, const vsomeip_sec_client_t* _sec_client);
    bool check_routing_credentials(const vsomeip_sec_client_t* _sec_client) const;
    void set_routing_credentials(uid_t _uid, gid_t _gid, const std::string& _name);

    bool is_client_allowed(const vsomeip_sec_client_t* _sec_client, service_t _service, instance_t _instance, method_t _method,
                           bool _is_request_service = false) const;
    bool is_offer_allowed(const vsomeip_sec_client_t* _sec_client, service_t _service, instance_t _instance) const;
...
#pragma warning(disable : 4251)
...
#endif
    mutable shared_mutex any_client_policies_mutex_;
    std::vector<std::shared_ptr<policy>> any_client_policies_;

    mutable shared_mutex is_client_allowed_cache_mutex_;
    ...
    mutable shared_mutex policy_extension_paths_mutex_;
        ...
        bool operator()(const vsomeip_sec_client_t& _lhs, const vsomeip_sec_client_t& _rhs) const {
            if (_lhs.port < _rhs.port) {
                ...
                if (_lhs.port == VSOMEIP_SEC_PORT_UNUSED) {
                    return ((_lhs.user < _rhs.user) || ((_lhs.user == _rhs.user) && (_lhs.group < _rhs.group)));
                } else {
                    return ((_lhs.host < _rhs.host) || ((_lhs.host == _rhs.host) && (_lhs.port < _rhs.port)));
                }
            }
            return false;
        }
