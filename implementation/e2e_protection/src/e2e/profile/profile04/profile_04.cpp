...
uint32_t profile_04::compute_crc(const profile_config& _config, const buffer_view& _buffer) {

    const buffer_view its_before = _buffer.subspan(0, _config.offset_ + 8);
    uint32_t computed_crc = e2e_crc::calculate_profile_04(its_before);

    if (_config.offset_ + 12 < _buffer.size()) {
        const buffer_view its_after = _buffer.subspan(_config.offset_ + 12);
