...
check_result profile_05_checker::check(buffer_view _buffer, instance_t _instance) {
    ...
    return make_check_result(status, _buffer, config_.offset_ + 3U);
}
...
bool profile_05_checker::read_8(buffer_view _buffer, uint8_t& _data, size_t _index) const {

    _data = _buffer[config_.offset_ + _index];
    return true;
}

bool profile_05_checker::read_16(buffer_view _buffer, uint16_t& _data, size_t _index) const {
