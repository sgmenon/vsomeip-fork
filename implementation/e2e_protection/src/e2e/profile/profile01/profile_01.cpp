...
uint8_t profile_01::compute_crc(const profile_config& _config, const buffer_view& _buffer) {
    ...
    case p01_data_id_mode::E2E_P01_DATAID_BOTH: // CRC over 2 bytes
        /*
         * Two bytes are included in the CRC (double ID configuration) This is used in E2E variant
         * 1A.
         */
    ...
    case p01_data_id_mode::E2E_P01_DATAID_ALT:
        ...
        if( counter % 2 == 0) {
             // CRC = Crc_CalculateCRC8(Config->DataID, 1, 0xFF, FALSE)
            computed_crc = crc::e2e_crc::calculate_profile_01(buffer::buffer_view(data_id_buffer, 1,
        2), 0xFF); //CRC over low byte of Data ID (LSB) } else {
            //  CRC = Crc_CalculateCRC8(Config->DataID >> 8,  1, 0xFF, FALSE)
            computed_crc = crc::e2e_crc::calculate_profile_01(buffer::buffer_view(data_id_buffer, 0,
        1), 0xFF); //CRC over high byte of Data ID (MSB)
        }
        */
        break;
    ...
    // CRC = CRC ^ 0xFF
    // To negate the last XOR 0xFF operation done on computed CRC by the last CalculateCRC8(), there
    // is a XORing doneexternally by E2E Library
