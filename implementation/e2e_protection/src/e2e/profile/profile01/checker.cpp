...
check_result profile_01_checker::check(buffer_view _buffer, instance_t _instance) {
    ...
    if (profile_01::is_buffer_length_valid(config_, _buffer)) {
        uint8_t received_crc = _buffer[config_.crc_offset_];
        uint8_t calculated_crc = profile_01::compute_crc(config_, _buffer);
    ...
    return make_check_result(status, _buffer, header_size(config_));
}
