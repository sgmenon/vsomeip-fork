...
bool local_tcp_client_endpoint_impl::send(const send_buffer_sequence_ptr_t& _sequence) {
...
void local_tcp_client_endpoint_impl::send_queued(std::pair<send_buffer_sequence_ptr_t, uint32_t>& _entry) {
...

    {
        ...
        if (socket_->is_open()) {
            socket_->async_write(bufs,
                                 strand_.wrap([self = shared_from_this(), sequence = _entry.first](auto ec, size_t length) {
                                     self->send_cbk(ec, length, sequence);
                                 }));
        } else {
