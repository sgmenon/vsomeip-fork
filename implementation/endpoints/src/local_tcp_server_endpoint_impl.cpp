...
local_tcp_server_endpoint_impl::local_tcp_server_endpoint_impl(const std::shared_ptr<endpoint_host>& _endpoint_host,
...
local_tcp_server_endpoint_impl::~local_tcp_server_endpoint_impl() = default;
bool local_tcp_server_endpoint_impl::is_local() const {

    return true;
}

void local_tcp_server_endpoint_impl::init(const endpoint_type& _local, boost::system::error_code& _error) {
    std::scoped_lock its_lock{acceptor_mutex_};
    init_unlocked(_local, _error);
}

void local_tcp_server_endpoint_impl::init_unlocked(const endpoint_type& _local, boost::system::error_code& _error) {
...
bool local_tcp_server_endpoint_impl::send(const buffer_sequence_ptr_t& _sequence) {
...
bool local_tcp_server_endpoint_impl::send_to(const std::shared_ptr<endpoint_definition> _target, const buffer_sequence_ptr_t& _sequence) {
...
bool local_tcp_server_endpoint_impl::send_error(const std::shared_ptr<endpoint_definition> _target, const byte_t* _data, uint32_t _size) {
...
bool local_tcp_server_endpoint_impl::send_queued(const target_data_iterator_type _it) {
...
bool local_tcp_server_endpoint_impl::get_default_target(service_t, local_tcp_server_endpoint_impl::endpoint_type&) const {
...
void local_tcp_server_endpoint_impl::accept_cbk(connection::ptr _connection, boost::system::error_code const& _error) {
        ...
        boost::system::error_code its_error;
        {
            ...
            if (its_error) {
                VSOMEIP_WARNING << "ltsei::" << __func__ << ": couldn't disable "
                                << "Nagle algorithm: " << its_error.message() << " endpoint > " << this;
            }
            ...
            if (its_error) {
                VSOMEIP_WARNING << "ltsei::" << __func__ << ": setting SO_LINGER failed (" << its_error.message() << ") " << this;
            }
            ...
            if (local_.address() == remote.address()) {
                // disable keep alive
                new_connection_socket.set_option(boost::asio::socket_base::keep_alive(false), its_error);
                if (its_error) {
                    VSOMEIP_WARNING << "ltsei::" << __func__ << ": couldn't disable "
    ...
    if (_error != boost::asio::error::bad_descriptor && _error != boost::asio::error::operation_aborted
        ...
        VSOMEIP_ERROR << "ltsei::" << __func__ << ": " << _error.message() << " (" << std::dec << _error.value()
        ...
        auto its_ep = std::dynamic_pointer_cast<local_tcp_server_endpoint_impl>(shared_from_this());
...
local_tcp_server_endpoint_impl::connection::connection(const std::shared_ptr<local_tcp_server_endpoint_impl>& _server,
    ...
    if (_server->is_routing_endpoint_ && !_server->configuration_->is_security_enabled()) {
...
local_tcp_server_endpoint_impl::connection::create(const std::shared_ptr<local_tcp_server_endpoint_impl>& _server,
                                                   std::uint32_t _max_message_size, std::uint32_t _buffer_shrink_threshold,
                                                   boost::asio::io_context& _io) {
    const std::uint32_t its_initial_buffer_size =
            static_cast<std::uint32_t>(protocol::COMMAND_HEADER_SIZE + sizeof(instance_t) + sizeof(bool) + sizeof(bool));
    return ptr(new connection(_server, _max_message_size, its_initial_buffer_size, _buffer_shrink_threshold, _io));
}

tcp_socket& local_tcp_server_endpoint_impl::connection::get_socket() {
    return *socket_;
}

std::unique_lock<std::mutex> local_tcp_server_endpoint_impl::connection::get_socket_lock() {
    ...
    if (socket_->is_open()) {
        ...
        if (recv_buffer_size_ > its_capacity) {
            VSOMEIP_ERROR << __func__ << "Received buffer size is greater than the buffer capacity!"
                          << " recv_buffer_size_: " << recv_buffer_size_ << " its_capacity: " << its_capacity << " endpoint > " << this;
            return;
        }
        ...
        socket_->async_receive(boost::asio::buffer(&recv_buffer_[recv_buffer_size_], left_buffer_size),
                               std::bind(&local_tcp_server_endpoint_impl::connection::receive_cbk, shared_from_this(),
...
void local_tcp_server_endpoint_impl::connection::send_queued(const buffer_sequence_ptr_t& _sequence) {
...

    {
        ...
        socket_->async_write(bufs,
                             std::bind(&local_tcp_server_endpoint_impl::connection::send_cbk, shared_from_this(), _sequence,
                                       std::placeholders::_1, std::placeholders::_2));
    }
}

client_t local_tcp_server_endpoint_impl::assign_client(const byte_t* _data, uint32_t _size) {
    ...
    if (its_error != protocol::error_e::ERROR_OK) {
        VSOMEIP_ERROR << __func__ << ": assign client command deserialization failed (" << std::dec << static_cast<int>(its_error) << ")";
        return VSOMEIP_CLIENT_UNSET;
    }

    return utility::request_client_id(configuration_, its_command.get_name(), its_command.get_client());
}
...
void local_tcp_server_endpoint_impl::get_configured_times_from_endpoint(service_t _service, method_t _method,
...
void local_tcp_server_endpoint_impl::connection::send_cbk(const buffer_sequence_ptr_t _sequence, boost::system::error_code const& _error,
                                                          std::size_t _bytes) {
    (void)_bytes;
    if (_sequence) {
...
void local_tcp_server_endpoint_impl::connection::receive_cbk(boost::system::error_code const& _error, std::size_t _bytes) {
    std::shared_ptr<local_tcp_server_endpoint_impl> its_server(server_.lock());
    if (!its_server) {
    ...
    if (_error == boost::asio::error::operation_aborted) {
        if (its_server->is_routing_endpoint_ && its_server->configuration_ && bound_client_ != VSOMEIP_CLIENT_UNSET) {
            utility::release_client_id(its_server->configuration_->get_network(), bound_client_);
    ...
    if (!_error && 0 < _bytes) {
        ...
        do {
            ...
            while (its_start + 3 < recv_buffer_size_ + its_iteration_gap
                   && (recv_buffer_[its_start] != 0x67 || recv_buffer_[its_start + 1] != 0x37 || recv_buffer_[its_start + 2] != 0x6d
            ...
            if (!message_is_empty) {
                ...
                if (its_command_size && max_message_size_ != MESSAGE_SIZE_UNLIMITED && its_command_size > max_message_size_) {
                    std::scoped_lock its_lock{socket_mutex_};
                    VSOMEIP_ERROR << "Received a local message which exceeds "
                                  << "maximum message size (" << std::dec << its_command_size << ") aborting! local: " << get_path_local()
                                  << " remote: " << get_path_remote() << " endpoint > " << this;
                    recv_buffer_.resize(recv_buffer_size_initial_, 0x0);
                    recv_buffer_.shrink_to_fit();
                    return;
                }
                ...
                while (its_end + 3 < recv_buffer_size_ + its_iteration_gap
                       && (recv_buffer_[its_end] != 0x07 || recv_buffer_[its_end + 1] != 0x6d || recv_buffer_[its_end + 2] != 0x37
                ...
                if (recv_buffer_size_ + its_iteration_gap < its_end + 4 || recv_buffer_[its_end] != 0x07
                    || recv_buffer_[its_end + 1] != 0x6d || recv_buffer_[its_end + 2] != 0x37 || recv_buffer_[its_end + 3] != 0x67) {
                    ...
                    if (its_start - its_iteration_gap + its_command_size + protocol::COMMAND_HEADER_SIZE + protocol::TAG_SIZE
                        > recv_buffer_size_) {
                        missing_capacity_ = std::uint32_t(its_start) - std::uint32_t(its_iteration_gap) + its_command_size
                                + std::uint32_t(protocol::COMMAND_HEADER_SIZE + protocol::TAG_SIZE) - std::uint32_t(recv_buffer_size_);
                        ...
                        missing_capacity_ =
                                static_cast<std::uint32_t>(protocol::COMMAND_HEADER_SIZE + protocol::TAG_SIZE - recv_buffer_size_);
                    } else {
                        std::stringstream local_msg;
                        local_msg << std::hex << std::setfill('0');
                        for (std::size_t i = its_iteration_gap; i < recv_buffer_size_ + its_iteration_gap && i - its_iteration_gap < 32;
                             i++) {
                            local_msg << std::setw(2) << static_cast<int>(recv_buffer_[i]) << " ";
                        }
                        VSOMEIP_ERROR << "ltsei::c<" << this << ">rcb: recv_buffer_size is: " << std::dec << recv_buffer_size_
                                      << " but couldn't read "
...
void local_tcp_server_endpoint_impl::connection::set_bound_client_host(const std::string& _bound_client_host) {
...
void local_tcp_server_endpoint_impl::connection::handle_recv_buffer_exception(const std::exception& _e) {
    std::stringstream its_message;
    its_message << "local_tcp_server_endpoint_impl::connection catched exception" << _e.what() << " local: " << get_path_local()
                << " remote: " << get_path_remote() << " shutting down connection. Start of buffer: " << std::hex << std::setfill('0');
...
std::size_t local_tcp_server_endpoint_impl::connection::get_recv_buffer_capacity() const {
...
void local_tcp_server_endpoint_impl::print_status() {
    ...
    VSOMEIP_INFO << "status lse: " << its_local_path << " connections: " << std::dec << its_connections.size() << " queues: " << std::dec
                 << targets_.size();
    for (const auto& c : its_connections) {
        ...
        VSOMEIP_INFO << "status lse: client: " << its_remote_path << " recv_buffer: " << std::dec << its_recv_size;
    }
}
std::string local_tcp_server_endpoint_impl::get_remote_information(const target_data_iterator_type _it) const {
    return _it->first.address().to_string() + ":" + std::to_string(_it->first.port());
}

std::string local_tcp_server_endpoint_impl::get_remote_information(const endpoint_type& _remote) const {
...
bool local_tcp_server_endpoint_impl::check_packetizer_space(buffer_sequence_ptr_t* _packetizer, std::uint32_t _size) const {
...
bool local_tcp_server_endpoint_impl::queue_train_buffer(target_data_iterator_type _it, buffer_sequence_ptr_t* _packetizer,
...
void local_tcp_server_endpoint_impl::send_client_identifier(const client_t& _client) {
    ...
    if (its_error != protocol::error_e::ERROR_OK) {
        ...
        VSOMEIP_ERROR << "ltsei::" << __func__ << ": assign client ack command serialization failed (" << std::dec
