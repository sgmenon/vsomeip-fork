    ...
    if (!_error) {
        if (!_event) {
            ...
            if (debounce_clients_.size() > 0) {
                debounce_timer.expires_after(std::chrono::duration_cast<std::chrono::milliseconds>(debounce_clients_.begin()->first
        ...
        bool notify_client{false};
        {
            ...
            if (its_subscribers.find(_client) != its_subscribers.end() && _filter) {
                ...
                int64_t elapsed = std::chrono::duration_cast<std::chrono::milliseconds>(its_current - _filter->last_forwarded_).count();
                is_elapsed = (_filter->last_forwarded_ == std::chrono::steady_clock::time_point::max() || elapsed >= _filter->interval_);
...
void routing_manager_base::register_event(client_t _client, service_t _service, instance_t _instance, event_t _notifier,
        ...
        if ((_is_shadow || is_routing_manager()) && !_epsilon_change_func) {
            ...
            if (its_debounce) {
                ...
                VSOMEIP_WARNING << "Using debounce configuration for "
                                << " SOME/IP event " << std::hex << std::setfill('0') << std::setw(4) << _service << "." << std::setw(4)
                                << _instance << "." << std::setw(4) << _notifier << "."
...
std::set<std::shared_ptr<event>> routing_manager_base::find_events(service_t _service, instance_t _instance,
...
std::vector<event_t> routing_manager_base::find_events(service_t _service, instance_t _instance) const {
...
bool routing_manager_base::is_response_allowed(client_t _sender, service_t _service, instance_t _instance, method_t _method) {
...
bool routing_manager_base::is_subscribe_to_any_event_allowed(const vsomeip_sec_client_t* _sec_client, client_t _client, service_t _service,
...
void routing_manager_base::unset_all_eventpayloads(service_t _service, instance_t _instance, eventgroup_t _eventgroup) {
    std::set<std::shared_ptr<event>> its_events;
    {
        std::lock_guard<std::mutex> its_lock(eventgroups_mutex_);
        const auto search = eventgroups_.find(service_instance_t{_service, _instance});
...
client_t routing_manager_base::find_local_client(service_t _service, instance_t _instance) const {
    std::lock_guard<std::mutex> its_lock(local_services_mutex_);
    return find_local_client_unlocked(_service, _instance);
}

client_t routing_manager_base::find_local_client_unlocked(service_t _service, instance_t _instance) const {
...
std::set<std::shared_ptr<eventgroupinfo>> routing_manager_base::find_eventgroups(service_t _service, instance_t _instance) const {
...
std::shared_ptr<eventgroupinfo> routing_manager_base::find_eventgroup(service_t _service, instance_t _instance,
                                                                      eventgroup_t _eventgroup) const {
    std::lock_guard<std::mutex> its_lock(eventgroups_mutex_);
...
void routing_manager_base::remove_eventgroup_info(service_t _service, instance_t _instance, eventgroup_t _eventgroup) {
    std::lock_guard<std::mutex> its_lock(eventgroups_mutex_);
    const auto search = eventgroups_.find(service_instance_t{_service, _instance});
...
void routing_manager_base::send_pending_subscriptions(service_t _service, instance_t _instance, major_version_t _major) {
...
void routing_manager_base::remove_pending_subscription(service_t _service, instance_t _instance, eventgroup_t _eventgroup, event_t _event) {
    std::scoped_lock its_lock(pending_subscription_mutex_);
