...
buffer_sequence_ptr_t compose_e2e_protected_sequence(const std::shared_ptr<e2e::e2e_provider>& _provider, const byte_t* _data,
    ...
    if (!_provider->is_protected({its_service, its_method})) {
        return nullptr;
    }
...
buffer_sequence_ptr_t compose_unprotected_forward_sequence(const owned_buffer_slice& _frame) {
    ...
    auto its_sequence = std::make_shared<buffer_sequence>();
    if (_frame.length > VSOMEIP_FULL_HEADER_SIZE) {
        its_sequence->append_buffer_slice(_frame.buffer, _frame.offset, VSOMEIP_FULL_HEADER_SIZE);
        its_sequence->append_buffer_slice(_frame.buffer, _frame.offset + VSOMEIP_FULL_HEADER_SIZE,
                                          _frame.length - VSOMEIP_FULL_HEADER_SIZE);
    } else {
        its_sequence->append_buffer_slice(_frame.buffer, _frame.offset, _frame.length);
    }
    return its_sequence;
}
...
void routing_manager_impl::start() {
    ...
    if (configuration_->log_status()) {
        std::scoped_lock its_lock{status_log_timer_mutex_};
        status_log_timer_.expires_after(std::chrono::seconds(0));
        status_log_timer_.async_wait(std::bind(&routing_manager_impl::status_log_timer_cbk, this, std::placeholders::_1));
    }
...
void routing_manager_impl::stop() {
    ...
    for (const auto& s : its_services) {
        for (const auto& i : s.second) {
            on_stop_offer_service(std::get<2>(i.second), s.first, i.first, std::get<0>(i.second), std::get<1>(i.second));
        }
    }
...
bool routing_manager_impl::offer_service(client_t _client, service_t _service, instance_t _instance, major_version_t _major,
...

    {
        ...
        if (is_external_routing_ready()) {
            ...
            VSOMEIP_INFO << "rmi::" << __func__ << ": added service 0x" << std::setw(4) << std::setfill('0') << std::hex << _service
                         << " to pending_sd_offers_.size = " << std::dec << pending_sd_offers_.size();
        }
    }
    ...
    if (is_local) {
        {
            ...
            for (auto it = pending_sd_offers_.begin(); it != pending_sd_offers_.end();) {
                if (it->first == _service && it->second == _instance) {
                    ...
                    VSOMEIP_INFO << "rmi::" << __func__ << " removed service: " << std::setw(4) << std::setfill('0') << std::hex << _service
                                 << " to pending_sd_offers_.size = " << std::dec << pending_sd_offers_.size();
                    break;
                } else {
                    ++it;
...
void routing_manager_impl::request_service(client_t _client, service_t _service, instance_t _instance, major_version_t _major,
    ...
    VSOMEIP_INFO << "REQUEST(" << std::hex << std::setfill('0') << std::setw(4) << _client << "): [" << std::setw(4) << _service << "."
                 << std::setw(4) << _instance << ":" << std::dec << int(_major) << "." << _minor << "]";
    ...
    if (!its_info) {
        ...
        if (discovery_) {
            if (!configuration_->is_local_service(_service, _instance)) {
                ...
                VSOMEIP_INFO << std::hex << "Avoid trigger SD find-service message"
                             << " for local service/instance/major/minor: " << _service << "/" << _instance << std::dec << "/"
...
void routing_manager_impl::release_service(client_t _client, service_t _service, instance_t _instance) {
    ...
    VSOMEIP_INFO << "RELEASE(" << std::hex << std::setfill('0') << std::setw(4) << _client << "): [" << std::setw(4) << _service << "."
...
void routing_manager_impl::subscribe(client_t _client, const vsomeip_sec_client_t* _sec_client, service_t _service, instance_t _instance,
    ...
    VSOMEIP_INFO << "SUBSCRIBE(" << std::hex << std::setfill('0') << std::setw(4) << _client << "): [" << std::setw(4) << _service << "."
                 << std::setw(4) << _instance << "." << std::setw(4) << _eventgroup << ":" << std::setw(4) << _event << ":" << std::dec
...
void routing_manager_impl::unsubscribe(client_t _client, const vsomeip_sec_client_t* _sec_client, service_t _service, instance_t _instance,
    ...
    VSOMEIP_INFO << "UNSUBSCRIBE(" << std::hex << std::setfill('0') << std::setw(4) << _client << "): [" << std::setw(4) << _service << "."
...
bool routing_manager_impl::send_with_sequence(client_t _client, buffer_sequence_ptr_t _sequence,
    ...
    if (is_request) {
        ...
        if (_client == get_client()) {
            ...
            is_sent = deliver_message(owned_buffer_slice::whole(its_flat), _instance, _reliable, _bound_client, _sec_client, _status_check,
                                      _sent_from_remote);
    ...
    if (its_target) {
        ...
        if ((its_client == host_->get_client() && is_response)
            ...
            is_sent = deliver_message(owned_buffer_slice::whole(its_trace_flat), _instance, _reliable, VSOMEIP_ROUTING_CLIENT, _sec_client,
                                      _status_check);
        } else {
            if (is_request) {
...
bool routing_manager_impl::send(client_t _client, message_buffer_ptr_t _frame, instance_t _instance, bool _reliable,
                                client_t _bound_client, const vsomeip_sec_client_t* _sec_client, uint8_t _status_check,
                                bool _sent_from_remote, bool _force) {

    bool is_sent(false);
    if (!_frame || _frame->size() <= VSOMEIP_MESSAGE_TYPE_POS) {
        return false;
    }

    const byte_t* _data = _frame->data();
    const length_t _size = static_cast<length_t>(_frame->size());
    ...
    if (is_request) {
        ...
        if (_client == get_client()) {
            deliver_message(owned_buffer_slice::whole(_frame), _instance, _reliable, _bound_client, _sec_client, _status_check,
                             _sent_from_remote);
            trace::header its_header;
            if (its_header.prepare(its_target, true, _instance))
    ...
    if (its_target) {
        is_sent = send_local(its_target, its_target_client, sequence_from_frame(_frame), _instance, _reliable, protocol::id_e::SEND_ID,
                             _status_check);
        if (is_sent
            && ((is_request && its_client == get_client()) || (is_response && find_local_client(its_service, _instance) == get_client())
                || (is_notification && find_local_client(its_service, _instance) == VSOMEIP_ROUTING_CLIENT))) {
        ...
        if ((its_client == host_->get_client() && is_response)
            ...
            is_sent = deliver_message(owned_buffer_slice::whole(_frame), _instance, _reliable, VSOMEIP_ROUTING_CLIENT, _sec_client,
                                      _status_check, false);
        } else {
            ...
            auto build_unprotected_sequence = [&]() -> buffer_sequence_ptr_t { return sequence_from_frame(_frame); };

            if (e2e_provider_) {
#ifndef ANDROID
                its_e2e_sequence = compose_e2e_protected_sequence(e2e_provider_, _data, _size, _instance, _frame);
                if (its_e2e_sequence) {
            ...
            if (is_request) {
                ...
                if (its_target) {
                    ...
                    VSOMEIP_ERROR << "Routing info for remote service could not be found! (" << std::hex << std::setfill('0')
                                  << std::setw(4) << its_client << "): [" << std::setw(4) << its_service << "." << std::setw(4)
                                  << _instance << "." << std::setw(4) << its_method << "] " << std::setw(4) << its_session;
                }
            } else {
                std::shared_ptr<serviceinfo> its_info(find_service(its_service, _instance));
                if (its_info || is_service_discovery) {
                                        ...
                                        if (its_eventgroup->is_sending_multicast()) {
                                            if (its_reliability == reliability_type_e::RT_UNRELIABLE
                                                ...
                                                if (its_eventgroup->get_multicast(its_address, its_port)) {
                                                    ...
                                                    its_multicast_target = endpoint_definition::get(its_address, its_port, false,
                                                                                                    its_service, _instance);
...
bool routing_manager_impl::send_via_sd(const std::shared_ptr<endpoint_definition>& _target, message_buffer_ptr_t _frame,
                                       uint16_t _sd_port) {
    ...
    buffer_sequence_ptr_t its_e2e_sequence;
    const byte_t* _data = _frame->data();
    const uint32_t _size = static_cast<uint32_t>(_frame->size());

    if (e2e_provider_) {
        VSOMEIP_DEBUG << "routing_manager_impl::send_via_sd() >>>> e2e_provider enabled, protecting SD message ..";
#ifndef ANDROID
        its_e2e_sequence = compose_e2e_protected_sequence(e2e_provider_, _data, _size, its_instance, _frame);
#endif
    }
    bool is_sent{false};
    ...
    if (its_endpoint) {
        auto its_sequence = its_e2e_sequence ? its_e2e_sequence : sequence_from_frame(_frame);
        is_sent = its_endpoint->send_to(_target, its_sequence);
...
void routing_manager_impl::register_event(client_t _client, service_t _service, instance_t _instance, event_t _notifier,
    ...
    VSOMEIP_INFO << "REGISTER EVENT(" << std::hex << std::setfill('0') << std::setw(4) << _client << "): [" << std::setw(4) << _service
                 << "." << std::setw(4) << _instance << "." << std::setw(4) << _notifier << ":is_provider=" << std::boolalpha
                 << _is_provided << "]";
}
...
bool routing_manager_impl::on_message(service_t _service, instance_t _instance, owned_buffer_slice _frame, bool _reliable,
                                      client_t _bound_client, const vsomeip_sec_client_t* _sec_client, uint8_t _check_status,
                                      bool _is_from_remote) {
#ifndef ANDROID
    return on_message_checked(_service, _instance, std::move(_frame), _reliable, _bound_client, _sec_client, _check_status, _is_from_remote,
                              nullptr);
#else
    if (!_frame.valid() || _frame.empty()) {
        return false;
    ...
    if (utility::is_notification(_data[VSOMEIP_MESSAGE_TYPE_POS])) {
        is_forwarded = deliver_notification(_service, _instance, _frame, _reliable, _bound_client, _sec_client, _check_status,
                                            _is_from_remote);
        ...
        send(its_client, std::move(its_send_frame), _instance, _reliable, _bound_client, _sec_client, _check_status, _is_from_remote,
             false);
    }
    return is_forwarded;
#endif
...
#ifndef ANDROID
bool routing_manager_impl::on_message_checked(service_t _service, instance_t _instance, owned_buffer_slice _frame, bool _reliable,
                                              client_t _bound_client, const vsomeip_sec_client_t* _sec_client, uint8_t _check_status,
                                              bool _is_from_remote, const e2e::check_result* _e2e_checked) {
    if (!_frame.valid() || _frame.empty()) {
        return false;
    }
    const byte_t* _data = _frame.data();
    ...
    if (utility::is_notification(_data[VSOMEIP_MESSAGE_TYPE_POS])) {
        is_forwarded = deliver_notification(_service, _instance, _frame, _reliable, _bound_client, _sec_client, _check_status,
                                            _is_from_remote, _e2e_checked);
    } else if (its_client == host_->get_client()) {
        if (_e2e_checked) {
            auto its_message = build_message_from_check_result(_frame, *_e2e_checked);
            if (its_message) {
        ...
        if (_e2e_checked) {
            its_forward = compose_e2e_stripped_sequence(_frame, *_e2e_checked);
        } else {
            its_forward = compose_unprotected_forward_sequence(_frame);
        }
        if (its_forward) {
            ...
            if (its_target) {
                ...
                send(its_client, its_flat, _instance, _reliable, _bound_client, _sec_client, _check_status, _is_from_remote, false);
            }
        }
    }
    return is_forwarded;
...
void routing_manager_impl::on_stop_offer_service(client_t _client, service_t _service, instance_t _instance, major_version_t _major,
    ...
    VSOMEIP_DEBUG << "rmi::ON_STOP_OFFER_SERVICE (" << std::hex << std::setfill('0') << std::setw(4) << _client << "): [" << std::setw(4)
...
bool routing_manager_impl::deliver_message(owned_buffer_slice _frame, instance_t _instance, bool _reliable, client_t _bound_client,
                                           const vsomeip_sec_client_t* _sec_client, uint8_t _status_check, bool _is_from_remote) {

    auto its_message = build_message_from_buffer(_frame);
    if (!its_message) {
    ...
    if (its_message) {
        ...
        if (!_is_from_remote) {
            if (utility::is_request(its_message->get_message_type())) {
                ...
                if (!is_offer_ok) {
                    VSOMEIP_WARNING << "vSomeIP Security: Client 0x" << std::hex << std::setw(4) << std::setfill('0') << get_client()
                ...
                if (is_intern_resp_allowed || is_offer_ok) {
                    ...
                    if (!is_access_member_ok && is_notification) {
                        VSOMEIP_WARNING << "vSomeIP Security: Client 0x" << std::hex << get_client()
                                        << " : routing_manager_impl::deliver_message: isn't "
                                           "allowed to receive a"
                                        << " notification from service/instance/method " << its_message->get_service() << "/"
                                        << its_message->get_instance() << "/" << its_message->get_method() << " respectively from client 0x"
            ...
            if (!configuration_->is_remote_access_allowed()) {
                ...
                VSOMEIP_WARNING << "vSomeIP Security: Client 0x" << std::hex << get_client()
                                << " : routing_manager_impl::deliver_message: " << std::hex << "Remote clients are not allowed"
                                << " to communicate with service/instance/method " << its_message->get_service() << "/"
                                << its_message->get_instance() << "/" << its_message->get_method() << " respectively with client 0x"
...
bool routing_manager_impl::deliver_notification(service_t _service, instance_t _instance, owned_buffer_slice _frame, bool _reliable,
                                                client_t _bound_client, const vsomeip_sec_client_t* _sec_client, uint8_t _status_check,
                                                bool _is_from_remote
#ifndef ANDROID
                                                ,
    ...
    const byte_t* _data = _frame.data();
    const length_t _length = static_cast<length_t>(_frame.length);
...
#ifndef ANDROID
        if (_e2e_checked) {
            its_payload = make_payload_from_span(_e2e_checked->app_payload, _frame);
        } else
#endif
        {
            const auto its_length = utility::get_payload_size(_data, _length);
            its_payload = make_payload_from_frame(_frame, VSOMEIP_PAYLOAD_POS, its_length);
        }
...
void routing_manager_impl::add_routing_info(service_t _service, instance_t _instance, major_version_t _major, minor_version_t _minor,
    ...
    if (!its_info) {
        ...
        VSOMEIP_ERROR << "rmi::" << __func__ << ": rejecting routing info. Remote: "
                      ...
                      << ((_reliable_port != ILLEGAL_PORT) ? _reliable_port : _unreliable_port) << " offered previously on this node: ["
                      << std::hex << std::setw(4) << _service << "." << std::setw(4) << _instance << "." << std::dec
    ...
    for (auto& s : get_services_remote()) {
        for (auto& i : s.second) {
            ...
            std::shared_ptr<client_endpoint> its_client_endpoint =
                    std::dynamic_pointer_cast<client_endpoint>(i.second->get_endpoint(_reliable));
    ...
    if (its_result) {
        if (!_subscription->is_pending()) { // resubscription without change
            ...
            if (its_reliable && its_unreliable)
                its_warning << "[";
            if (its_reliable)
                its_warning << its_reliable->get_address().to_string() << ":" << std::dec << its_reliable->get_port();
            if (its_reliable && its_unreliable)
                its_warning << ", ";
            if (its_unreliable)
                its_warning << its_unreliable->get_address().to_string() << ":" << std::dec << its_unreliable->get_port();
            if (its_reliable && its_unreliable)
                its_warning << "]";
...
std::chrono::steady_clock::time_point routing_manager_impl::expire_subscriptions(bool _force) {
    ...
    for (auto& s : its_expired_subscriptions) {
        ...
        if (its_info) {
            ...
            remote_subscription_id_t its_id;
            std::unique_lock<std::mutex> its_update_lock{update_remote_subscription_mutex_};
            auto its_result = its_info->update_remote_subscription(s.first, std::chrono::steady_clock::now(), s.second, its_id, false);
            if (its_result) {
                const client_t its_offering_client = find_local_client(its_service, its_instance);
            ...
            if (s.first->get_unreliable()) {
                VSOMEIP_INFO << (_force ? "Removed" : "Expired") << " subscription [" << std::hex << std::setfill('0') << std::setw(4)
                             << its_service << "." << std::setw(4) << its_instance << "." << std::setw(4) << its_eventgroup
                             << "] unreliable from " << s.first->get_unreliable()->get_address() << ":" << std::dec
                             << s.first->get_unreliable()->get_port();
            }

            if (s.first->get_reliable()) {
                VSOMEIP_INFO << (_force ? "Removed" : "Expired") << " subscription [" << std::hex << std::setfill('0') << std::setw(4)
                             << its_service << "." << std::setw(4) << its_instance << "." << std::setw(4) << its_eventgroup
                             << "] reliable from " << s.first->get_reliable()->get_address() << ":" << std::dec
                             << s.first->get_reliable()->get_port();
            }
        }
    }
                    ...
                    if (!already_pinged) {
                        ...
                        if (its_old_endpoint) {
                            ...
                            if (stub_ && stub_->send_ping(its_stored_client)) {
                                ...
                                VSOMEIP_WARNING << "OFFER(" << std::hex << std::setfill('0') << std::setw(4) << _client << "): ["
                                                << std::setw(4) << _service << "." << std::setw(4) << _instance << ":" << std::dec
                                                << int(_major) << "." << std::dec << _minor
                                                << "] is now pending. Waiting for pong from application: " << std::hex << std::setw(4)
                                                << its_stored_client;
                                return false;
                            }
                            ...
                            VSOMEIP_ERROR << "rmi::" << __func__ << ": rejecting service registration. Application: " << std::hex
                                          ...
                                          << std::setw(4) << _instance << "." << std::dec << static_cast<std::uint32_t>(its_stored_major)
    ...
    for (auto service_iter = pending_offers_.begin(); service_iter != pending_offers_.end();) {
        for (auto instance_iter = service_iter->second.begin(); instance_iter != service_iter->second.end();) {
            if (std::get<3>(instance_iter->second) == _client) {
                ...
                VSOMEIP_WARNING << "OFFER(" << std::hex << std::setfill('0') << std::setw(4) << std::get<2>(instance_iter->second) << "): ["
                                << std::setw(4) << service_iter->first << "." << std::setw(4) << instance_iter->first << ":" << std::dec
                                << std::uint32_t(std::get<0>(instance_iter->second)) << "." << std::get<1>(instance_iter->second)
                                << "] was rejected as application: " << std::hex << std::setw(4) << _client << " is still alive";
...
void routing_manager_impl::handle_client_error(client_t _client) {
    VSOMEIP_INFO << "rmi::" << __func__ << " Client 0x" << std::hex << std::setw(4) << std::setfill('0') << get_client()
                 << " handles a client error 0x" << std::setw(4) << std::hex << _client;
    if (stub_)
    ...
    std::forward_list<std::tuple<client_t, service_t, instance_t, major_version_t, minor_version_t>> its_offers;
    {
        ...
        for (auto service_iter = pending_offers_.begin(); service_iter != pending_offers_.end();) {
            for (auto instance_iter = service_iter->second.begin(); instance_iter != service_iter->second.end();) {
                if (std::get<3>(instance_iter->second) == _client) {
                    VSOMEIP_WARNING << "OFFER(" << std::hex << std::setfill('0') << std::setw(4) << std::get<2>(instance_iter->second)
                                    << "): [" << std::setw(4) << service_iter->first << "." << std::setw(4) << instance_iter->first << ":"
                                    << std::dec << std::uint32_t(std::get<0>(instance_iter->second)) << "."
                                    << std::get<1>(instance_iter->second) << "] is not pending anymore as application: " << std::hex
                                    << std::setw(4) << std::get<3>(instance_iter->second) << " is dead. Offering again!";
                    its_offers.push_front(std::make_tuple(std::get<2>(instance_iter->second), service_iter->first, instance_iter->first,
                                                          std::get<0>(instance_iter->second), std::get<1>(instance_iter->second)));
                    instance_iter = service_iter->second.erase(instance_iter);
                } else {
                    ++instance_iter;
                }
    ...
    for (const auto& offer : its_offers) {
        offer_service(std::get<0>(offer), std::get<1>(offer), std::get<2>(offer), std::get<3>(offer), std::get<4>(offer), true);
    }
}
                ...
                for (const auto& its_instance : its_service.second) {
                    ...
                    if (has_reliable || has_unreliable) {
                        ...
                        if (its_client == VSOMEIP_ROUTING_CLIENT) {
                            ...
                            del_routing_info(its_service.first, its_instance.first, has_reliable, has_unreliable, true);
...
std::set<client_t> routing_manager_impl::get_requesters_unlocked(service_t _service, instance_t _instance, major_version_t _major,
    ...
    for (const auto& its_major : found_instance->second) {
        if (its_major.first == _major || _major == DEFAULT_MAJOR || its_major.first == ANY_MAJOR) {
            for (const auto& its_minor : its_major.second) {
                if (its_minor.first <= _minor || _minor == DEFAULT_MINOR || its_minor.first == ANY_MINOR) {
                    if (its_requesters.empty()) {
                        its_requesters = its_minor.second;
                    } else {
                        its_requesters.insert(its_minor.second.cbegin(), its_minor.second.cend());
                    }
                }
            }
        }
...
void routing_manager_impl::status_log_timer_cbk(boost::system::error_code const& _error) {
    ...
    {
        std::scoped_lock its_lock{status_log_timer_mutex_};
        status_log_timer_.expires_after(std::chrono::seconds(configuration_->get_log_status_interval()));
        status_log_timer_.async_wait(std::bind(&routing_manager_impl::status_log_timer_cbk, this, std::placeholders::_1));
    }
}
...
void routing_manager_impl::send_subscription(const client_t _offering_client, const service_t _service, const instance_t _instance,
    ...
    if (host_->get_client() == _offering_client) {
        ...
        for (const auto its_client : _clients) {
            if (stub_
                && !stub_->send_subscribe(find_local(_offering_client), its_client, _service, _instance, _eventgroup, _major, ANY_EVENT,
                                          nullptr, _id)) {
                try {
...
void routing_manager_impl::send_unsubscription(client_t _offering_client, service_t _service, instance_t _instance,
    ...
    if (host_->get_client() == _offering_client) {
        ...
        for (const auto its_client : _removed) {
            if (stub_
                && !stub_->send_unsubscribe(find_local(_offering_client), its_client, _service, _instance, _eventgroup, ANY_EVENT, _id)) {
                try {
...
void routing_manager_impl::send_expired_subscription(client_t _offering_client, service_t _service, instance_t _instance,
    ...
    if (host_->get_client() == _offering_client) {
        ...
        for (const auto its_client : _removed) {
            if (stub_)
                stub_->send_expired_subscription(find_local(_offering_client), its_client, _service, _instance, _eventgroup, ANY_EVENT,
                                                 _id);
        }
    }
}
        ...
        std::stringstream its_log;
        {
            ...
            for (const auto& s : message_statistics_) {
                if (s.second.counter_ / (its_interval / 1000) > its_min_freq) {
                    ...
                    its_log << std::hex << std::setfill('0') << std::setw(4) << std::get<0>(s.first) << "." << std::get<1>(s.first) << "."
                            << std::get<2>(s.first) << ": #=" << std::dec << s.second.counter_ << " L=" << s.second.avg_length_
                            << " S=" << std::dec << its_subscribed << ", ";
                }
            }
