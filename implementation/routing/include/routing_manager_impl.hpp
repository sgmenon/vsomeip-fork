...
public:
    ...
    bool send_via_sd(const std::shared_ptr<endpoint_definition>& _target, message_buffer_ptr_t _frame, uint16_t _sd_port);
    ...
    void on_subscribe_nack(client_t _client, service_t _service, instance_t _instance, eventgroup_t _eventgroup, bool _remove,
                           remote_subscription_id_t _id);
    ...
    void on_subscribe_ack_with_multicast(service_t _service, instance_t _instance, const boost::asio::ip::address& _sender,
...
private:
    ...
    std::set<eventgroup_t> get_subscribed_eventgroups(service_t _service, instance_t _instance);
    ...
    void send_subscription(const client_t _offering_client, const service_t _service, const instance_t _instance,
                           const eventgroup_t _eventgroup, const major_version_t _major, const std::set<client_t>& _clients,
                           const remote_subscription_id_t _id);

    void send_unsubscription(client_t _offering_client, const service_t _service, const instance_t _instance,
                             const eventgroup_t _eventgroup, const major_version_t _major, const std::set<client_t>& _removed,
                             const remote_subscription_id_t _id);
...
private:
    ...
    std::map<service_t, std::map<instance_t, std::map<major_version_t, std::map<minor_version_t, std::set<client_t>>>>> requested_services_;
    ...
    std::map<std::tuple<service_t, instance_t, method_t>, msg_statistic_t> message_statistics_;
