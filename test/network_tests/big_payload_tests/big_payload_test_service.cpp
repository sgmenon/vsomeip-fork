...
void big_payload_test_service::offer() {
    app_->offer_service(service_id_, big_payload_test::TEST_SERVICE_INSTANCE_ID);
}

void big_payload_test_service::stop_offer() {
    app_->stop_offer_service(service_id_, big_payload_test::TEST_SERVICE_INSTANCE_ID);
}
        ...
        while (!blocked_ || !incoming_requests_.empty()) {
            ...
            if (test_mode_ == big_payload_test::test_mode::RANDOM) {
                ...
                if (number_of_received_messages_ % 2) {
                    ...
                    // as well
                    its_payload_data.assign(big_payload_test::BIG_PAYLOAD_SIZE + 2048, big_payload_test::DATA_SERVICE_TO_CLIENT);
                } else {
                    its_payload_data.assign(big_payload_test::BIG_PAYLOAD_SIZE, big_payload_test::DATA_SERVICE_TO_CLIENT);
                }
            } else if (test_mode_ == big_payload_test::test_mode::UDP) {
                its_payload_data.assign(big_payload_test::BIG_PAYLOAD_SIZE_UDP, big_payload_test::DATA_SERVICE_TO_CLIENT);
            } else {
                its_payload_data.assign(big_payload_test::BIG_PAYLOAD_SIZE, big_payload_test::DATA_SERVICE_TO_CLIENT);
            }
