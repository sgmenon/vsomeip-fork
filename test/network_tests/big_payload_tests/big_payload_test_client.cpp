...
big_payload_test_client::big_payload_test_client(bool _use_tcp, big_payload_test::test_mode _test_mode) :
    ...
    request_(vsomeip::runtime::get()->create_request(_use_tcp)), blocked_(false), is_available_(false), test_mode_(_test_mode),
    number_of_messages_to_send_(test_mode_ == big_payload_test::test_mode::RANDOM
                                        ? big_payload_test::BIG_PAYLOAD_TEST_NUMBER_MESSAGES_RANDOM
                                        : big_payload_test::BIG_PAYLOAD_TEST_NUMBER_MESSAGES),
    number_of_sent_messages_(0), number_of_acknowledged_messages_(0), sender_(std::bind(&big_payload_test_client::run, this)) {
    switch (test_mode_) {
...
bool big_payload_test_client::init() {
    ...
    app_->register_state_handler(std::bind(&big_payload_test_client::on_state, this, std::placeholders::_1));

    app_->register_message_handler(vsomeip::ANY_SERVICE, vsomeip::ANY_INSTANCE, vsomeip::ANY_METHOD,
                                   std::bind(&big_payload_test_client::on_message, this, std::placeholders::_1));

    app_->register_availability_handler(service_id_, big_payload_test::TEST_SERVICE_INSTANCE_ID,
                                        std::bind(&big_payload_test_client::on_availability, this, std::placeholders::_1,
...
void big_payload_test_client::on_state(vsomeip::state_type_e _state) {
    if (_state == vsomeip::state_type_e::ST_REGISTERED) {
        app_->request_service(service_id_, big_payload_test::TEST_SERVICE_INSTANCE_ID, false);
    }
}

void big_payload_test_client::on_availability(vsomeip::service_t _service, vsomeip::instance_t _instance, bool _is_available) {
    VSOMEIP_INFO << "Service [" << std::hex << std::setfill('0') << std::setw(4) << _service << "." << _instance << "] is "
                 << (_is_available ? "available." : "NOT available.");

    if (service_id_ == _service && big_payload_test::TEST_SERVICE_INSTANCE_ID == _instance) {
...
void big_payload_test_client::run() {
    ...
    for (unsigned int i = 0; i < number_of_messages_to_send_; i++) {
        if (test_mode_ == big_payload_test::test_mode::RANDOM) {
        ...
        } else if (test_mode_ == big_payload_test::test_mode::LIMITED || test_mode_ == big_payload_test::test_mode::LIMITED_GENERAL
                   || test_mode_ == big_payload_test::test_mode::QUEUE_LIMITED_GENERAL
                   || test_mode_ == big_payload_test::test_mode::QUEUE_LIMITED_SPECIFIC) {
            if (i % 2) {
            ...
            its_payload_data.assign(big_payload_test::BIG_PAYLOAD_SIZE_UDP, big_payload_test::DATA_CLIENT_TO_SERVICE);
        } else {
            its_payload_data.assign(big_payload_test::BIG_PAYLOAD_SIZE, big_payload_test::DATA_CLIENT_TO_SERVICE);
        }
