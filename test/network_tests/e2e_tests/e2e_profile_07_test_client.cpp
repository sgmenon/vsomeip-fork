...
e2e_profile_07_test_client::e2e_profile_07_test_client() :
    app_(vsomeip::runtime::get()->create_application()), is_available_(false), sender_(std::bind(&e2e_profile_07_test_client::run, this)),
    received_(0) { }
...
bool e2e_profile_07_test_client::init() {
    ...
    app_->register_state_handler(std::bind(&e2e_profile_07_test_client::on_state, this, std::placeholders::_1));

    app_->register_message_handler(PROFILE_07_SERVICE, PROFILE_07_INSTANCE, vsomeip::ANY_METHOD,
                                   std::bind(&e2e_profile_07_test_client::on_message, this, std::placeholders::_1));

    app_->register_availability_handler(PROFILE_07_SERVICE, PROFILE_07_INSTANCE,
                                        std::bind(&e2e_profile_07_test_client::on_availability, this, std::placeholders::_1,
...
void e2e_profile_07_test_client::on_state(vsomeip::state_type_e _state) {
    ...
    if (_state == vsomeip::state_type_e::ST_REGISTERED) {
        ...
        app_->request_event(PROFILE_07_SERVICE, PROFILE_07_INSTANCE, PROFILE_07_EVENT, {PROFILE_07_EVENTGROUP},
...
void e2e_profile_07_test_client::on_availability(vsomeip::service_t _service, vsomeip::instance_t _instance, bool _is_available) {
    ...
    if (PROFILE_07_SERVICE == _service && PROFILE_07_INSTANCE == _instance) {
        ...
        if (is_available_ != !_is_available) {
            ...
            app_->subscribe(PROFILE_07_SERVICE, PROFILE_07_INSTANCE, PROFILE_07_EVENTGROUP, PROFILE_07_MAJOR);
            std::this_thread::sleep_for(std::chrono::milliseconds(1000));
...
void e2e_profile_07_test_client::on_message(const std::shared_ptr<vsomeip::message>& _message) {
    ...
    } else if (vsomeip::message_type_e::MT_NOTIFICATION == _message->get_message_type() && PROFILE_07_EVENT == _message->get_method()) {
...
void e2e_profile_07_test_client::run() {
...
void e2e_profile_07_test_client::join_sender_thread() {
...
void e2e_profile_07_test_client::shutdown_service() {
