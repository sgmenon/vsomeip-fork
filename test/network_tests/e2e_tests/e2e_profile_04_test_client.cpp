...
e2e_profile_04_test_client::e2e_profile_04_test_client() :
    app_(vsomeip::runtime::get()->create_application()), is_available_(false), sender_(std::bind(&e2e_profile_04_test_client::run, this)),
    received_(0) { }

bool e2e_profile_04_test_client::init() {
    ...
    app_->register_state_handler(std::bind(&e2e_profile_04_test_client::on_state, this, std::placeholders::_1));

    app_->register_message_handler(PROFILE_04_SERVICE, PROFILE_04_INSTANCE, vsomeip::ANY_METHOD,
                                   std::bind(&e2e_profile_04_test_client::on_message, this, std::placeholders::_1));

    app_->register_availability_handler(PROFILE_04_SERVICE, PROFILE_04_INSTANCE,
                                        std::bind(&e2e_profile_04_test_client::on_availability, this, std::placeholders::_1,
...
void e2e_profile_04_test_client::on_state(vsomeip::state_type_e _state) {
    ...
    if (_state == vsomeip::state_type_e::ST_REGISTERED) {
        ...
        app_->request_event(PROFILE_04_SERVICE, PROFILE_04_INSTANCE, PROFILE_04_EVENT, {PROFILE_04_EVENTGROUP},
...
void e2e_profile_04_test_client::on_availability(vsomeip::service_t _service, vsomeip::instance_t _instance, bool _is_available) {
    ...
    if (PROFILE_04_SERVICE == _service && PROFILE_04_INSTANCE == _instance) {
        ...
        if (is_available_ && !_is_available) {
            ...
            app_->subscribe(PROFILE_04_SERVICE, PROFILE_04_INSTANCE, PROFILE_04_EVENTGROUP, PROFILE_04_MAJOR);
            std::this_thread::sleep_for(std::chrono::milliseconds(1000));
...
void e2e_profile_04_test_client::on_message(const std::shared_ptr<vsomeip::message>& _message) {
    ...
    if (vsomeip::message_type_e::MT_RESPONSE == _message->get_message_type() && PROFILE_04_METHOD == _message->get_method()) {
        ...
        // hole-free app payload after E2E strip
        ...
        for (size_t i = 0; i < its_payload->get_length(); i++)
            EXPECT_EQ(its_data[i], responses_[counters_[PROFILE_04_METHOD] % PROFILE_O4_NUM_MESSAGES][i]);

        counters_[PROFILE_04_METHOD]++;

    } else if (vsomeip::message_type_e::MT_NOTIFICATION == _message->get_message_type() && PROFILE_04_EVENT == _message->get_method()) {
        ...
        const auto its_data = its_payload->get_data();
        for (size_t i = 0; i < its_payload->get_length(); i++)
            EXPECT_EQ(its_data[i], events_[counters_[PROFILE_04_EVENT] % PROFILE_O4_NUM_MESSAGES][i]);
...
void e2e_profile_04_test_client::run() {
...
void e2e_profile_04_test_client::join_sender_thread() {
...
void e2e_profile_04_test_client::shutdown_service() {
...
int main(int argc, char** argv) {
    ...
    // Hole-free app payloads (E2E header stripped on receive)
