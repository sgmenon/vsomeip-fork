...
class common {
public:
    common() : instance_id_{0}, service_id_{0}, major_version_{0}, minor_version_{0}, availability_{false}, msg_sent_{false} { }
...
class client : common {
public:
    client(service_t _service_id, instance_t _instance_id, major_version_t _major_version, minor_version_t _minor_version) {
        ...
        app_->register_message_handler(service_id_, instance_id_, ANY_METHOD, std::bind(&client::on_message, this, std::placeholders::_1));
    ...
    void on_message(const std::shared_ptr<message>& _message) {
        ...
        std::stringstream msg;
        {
            ...
            for (uint32_t i = 0; i < len; ++i) {
                ...
                msg << std::hex << std::setfill('0') << std::setw(2) << static_cast<int>((*(its_payload->get_data() + i))) << " ";
            }
        }
...
class server : common {
public:
    server(service_t _service_id, instance_t _instance_id, major_version_t _major_version, minor_version_t _minor_version) {
        service_id_ = _service_id;
        instance_id_ = _instance_id;
        major_version_ = _major_version;
        minor_version_ = _minor_version;
        ...
        app_->register_availability_handler(
                service_id_, instance_id_,
                std::bind(&server::on_availability, this, std::placeholders::_1, std::placeholders::_2, std::placeholders::_3));
        app_->register_message_handler(service_id_, instance_id_, ANY_METHOD, std::bind(&server::on_message, this, std::placeholders::_1));
